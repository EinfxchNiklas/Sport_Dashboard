...
from data_sources.bvb_alt import get_bvb_injuries, get_bvb_recent_and_upcoming_matches
...
with tab_bvb:
    ...
    sub_spielplan, sub_injuries, sub_stats, sub_recent_upcoming = st.tabs([
        "📅 Spielplan", "Verletztenliste", "📊 Statistiken", "Letzte & Nächste Spiele"
    ])
    ...
    with sub_spielplan:
        ...
        matches_by_comp = get_bvb_recent_and_upcoming_matches() or {}
        ...
        if not matches_by_comp:
            ...
            comp_names = list(matches_by_comp)
    ...
    with sub_recent_upcoming:
        st.subheader("Letzte 4 und nächste 5 Spiele")

        try:
            past_matches, upcoming_matches = get_bvb_recent_and_upcoming_matches()

            st.markdown("### Letzte 4 Spiele")
            if not past_matches:
                st.info("Keine vergangenen Spiele gefunden.")
            else:
                for match in past_matches:
                    st.write(f"{match['Datum']}: {match['Heim']} vs {match['Auswärts']} - {match['Ergebnis']} ({match['Liga']})")

            st.markdown("### Nächste 5 Spiele")
